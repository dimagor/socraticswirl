...
```
You should see "Initialization completed.", which indicates the initialization process succeeded. You only need to initialize Socraticswirl once on your personal computer, as long as the keys and the instructor are not changed. However, if you are using a shared computer, it is highly recommened that you initialize Socraticswirl again each time, just make sure the Socraticswirl software on this shared computer is not initialized under someone else. 
